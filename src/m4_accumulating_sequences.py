...
def run_test_make_simple_string():
    ...
    #Test 1
    expected = str('10-11-12-13-')
    actual = make_simple_string(10, 13)
    print('Expected:', expected)
    print('Actual:  ', actual)

    # Test 2
    expected = str('11-12-13-')
...
def rectangles_from_circles(circles):
    ...
    for k in range(len(circles)):
        left_top = rg.Point(circles[k].center.x-circles[k].radius,circles[
            k].center.y-circles[k].radius)
        right_bottom = rg.Point(circles[k].center.x+circles[k].radius,circles[
            k].center.y+circles[k].radius)
        rectangle = rg.Rectangle(left_top,right_bottom)
